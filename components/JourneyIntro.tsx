...
export default function JourneyIntro({ onStart }: JourneyIntroProps) {
  ...
  return (
    <section id="intro" className="flex min-h-[calc(100vh-96px)] w-full items-center justify-center bg-[var(--omni-bg-main)] px-6 py-16">
      <div className="w-full max-w-3xl space-y-6 rounded-[16px] border border-[var(--omni-border-soft)] bg-[var(--omni-surface-card)]/92 px-8 py-12 text-center shadow-[0_20px_45px_rgba(0,0,0,0.08)] backdrop-blur-[2px]">
        <MultiTypewriter
          key={introLines.join("|")}
          lines={introLines}
          speed={60}
          gapMs={500}
          wrapperClassName="typewriter-display mb-6 w-full max-w-[62ch] mx-auto bg-[var(--omni-bg-paper)] px-6 py-5"
          headingClassName="text-base md:text-lg"
        />

        {/* Ce urmează */}
        <div className="mx-auto mt-2 max-w-xl text-left">
          <h2 className="mb-2 text-base font-semibold text-[var(--omni-ink)]">{s("journeyWhatsNext", lang === 'ro' ? "Ce urmează" : "What’s next") as string}</h2>
          <ul className="space-y-2 text-sm leading-relaxed text-[var(--omni-ink)]">
        ...
        <div className="mx-auto mt-3 max-w-xl text-left">
          <h3 className="mb-1 text-sm font-semibold text-[var(--omni-ink)]">{s("journeyWhatNot", lang === 'ro' ? "Ce NU este / Garanții" : "What it isn’t / Guarantees") as string}</h3>
          <p className="text-sm leading-relaxed text-[var(--omni-ink)]/80">
            {lang === 'ro'
