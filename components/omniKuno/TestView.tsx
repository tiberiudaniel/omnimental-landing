    ...
    <section className="space-y-4 rounded-card border border-[var(--omni-border-soft)] bg-[var(--omni-surface-card)] px-6 py-5 text-[var(--omni-ink)]">
      ...
      <div className="space-y-3">
        ...
        {current.type === "reflectionShort" ? (
          <p className="text-[11px] text-[var(--omni-muted)]">Răspunsul tău rămâne privat, folosit doar pentru recapitulare personală.</p>
        ) : null}
...
function renderQuestion(
  ...
  if (question.type === "singleChoice" || question.type === "scenario") {
    return (
      <div className="space-y-3">
        <p className="text-base font-semibold text-[var(--omni-ink)]">{question.question}</p>
  ...
  if (question.type === "reflectionShort") {
    return (
      <div className="space-y-3">
        <p className="text-base font-semibold text-[var(--omni-ink)]">{question.question}</p>
        <p className="text-sm text-[var(--omni-muted)]">{question.prompt}</p>
        <input
          type="text"
          value={typeof stored === "string" ? stored : ""}
          onChange={(event) => setAnswers({ ...answers, [id]: event.target.value })}
