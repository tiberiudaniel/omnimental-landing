  ...
  return (
    ...
    >
      <div className="space-y-3 text-sm text-[#4D3F36]">
        {arcLevel ? (
          <div className="inline-flex items-center gap-2 rounded-full border border-[#EADFD4] bg-[#FFF9F3] px-3 py-1 text-[12px] font-semibold text-[#5A3E2B]">
            {lang === "ro" ? `Nivel ${arcLevel} – Arc 1` : `Level ${arcLevel} – Arc 1`}
          </div>
        ) : null}
        <div>
          <p className="text-[12px] font-semibold text-[#2D2017]">
            {lang === "ro" ? "Progres lecții" : "Lesson progress"}
          </p>
          <div className="mt-1 h-2 rounded-full bg-[#F2E7DD]">
            <div
              className="h-2 rounded-full bg-[#C07963]"
        ...
        <div className="grid grid-cols-3 gap-2 text-[11px] text-[#7B6B60]">
          <div className="rounded-2xl border border-[#EADFD4] bg-[#FFFBF7] px-2 py-2 text-center">
            <p className="text-[18px] font-semibold text-[#2D2017]">{abilDailyCount}</p>
            <p>{lang === "ro" ? "Daily" : "Daily"}</p>
          </div>
          <div className="rounded-2xl border border-[#EADFD4] bg-[#FFFBF7] px-2 py-2 text-center">
            <p className="text-[18px] font-semibold text-[#2D2017]">{abilWeeklyCount}</p>
            <p>{lang === "ro" ? "Weekly" : "Weekly"}</p>
          </div>
          <div className="rounded-2xl border border-[#EADFD4] bg-[#FFFBF7] px-2 py-2 text-center">
            <p className="text-[18px] font-semibold text-[#2D2017]">{dailyStreak}</p>
