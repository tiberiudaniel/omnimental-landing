...
export function OmniAbilCard({ lang }: { lang: string }) {
  ...
  const renderTask = (task: OmniAbilTask | null, type: "daily" | "weekly") => {
    if (!task) {
      ...
      return <p className="text-sm text-[#7B6B60]">{fallback}</p>;
    }
    const isDone = task.status === "done";
    const xpLabel = task.xpReward ? `+${task.xpReward} XP` : null;
    return (
      <div className="rounded-2xl border border-[#EFE4D8] bg-white/90 px-3 py-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[13px] font-semibold text-[#2D2017]">{task.title}</p>
            {task.description ? <p className="text-[11px] text-[#7B6B60]">{task.description}</p> : null}
            {xpLabel ? <p className="mt-1 text-[10px] uppercase tracking-[0.2em] text-[#B08A78]">{xpLabel}</p> : null}
          </div>
          <div className="flex flex-col items-end gap-2">
            <span
              className={`text-[10px] font-semibold uppercase tracking-[0.2em] ${
                isDone ? "text-[#1F7A43]" : "text-[#B08A78]"
              }`}
            >
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] transition ${
                isDone
                  ...
                  : "border border-[#C5B29E] text-[#5A3E2B] hover:border-[#8B5A3A]"
              } disabled:opacity-60`}
            >
  ...
  return (
    <Card className="rounded-2xl border border-[#E4DAD1] bg-white p-3 shadow-sm sm:p-4">
      ...
      {loading ? (
        <p className="text-sm text-[#7B6B60]">{lang === "ro" ? "Se încarcă misiunile..." : "Loading missions..."}</p>
      ) : isEmpty ? (
