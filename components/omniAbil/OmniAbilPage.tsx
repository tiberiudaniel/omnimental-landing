    ...
    <article className="flex h-full flex-col rounded-card border border-[var(--omni-border-soft)] bg-[var(--omni-surface-card)]/95 p-4 shadow-[0_10px_24px_rgba(0,0,0,0.06)]">
      <header className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-[15px] font-semibold text-[var(--omni-ink)]">{t(ability.title)}</h3>
          <p className="mt-1 text-[12px] leading-relaxed text-[var(--omni-muted)]">{t(ability.oneLiner)}</p>
        </div>
      </header>
...
export default function OmniAbilPage() {
  ...
  const abilitiesWithMoves = getUserAbilitiesWithMovesFromContext({ missionCategory });
  ...
  const title =
    normalizedLang === "ro" ? "OmniAbil · Implementare practică" : "OmniAbil · Practical implementation";
    ...
    <>
      ...
      <AppShell header={header}>
        <section className="omni-card rounded-card bg-[var(--omni-surface-card)]/95 p-6 shadow-[0_16px_40px_rgba(0,0,0,0.06)] md:p-7">
          ...
          <p className="mt-3 text-sm text-[var(--omni-ink-soft)] sm:text-base">{subtitle}</p>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
          ...
          <div className="omni-card h-full rounded-card bg-[var(--omni-surface-card)]/95 p-4 shadow-[0_12px_28px_rgba(0,0,0,0.06)] sm:p-5">
            ...
            <ul className="mt-3 space-y-2 text-[13px] leading-relaxed text-[var(--omni-ink-soft)]">
              <li>{normalizedLang === "ro" ? "1. OmniKuno îți oferă claritate și modele mentale." : "1. OmniKuno gives you clarity and mental models."}</li>
              <li>
                {normalizedLang === "ro"
        ...
        <section className="mt-8">
          <div className="mb-3 flex flex-wrap items-baseline justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[var(--omni-muted)]">
                {normalizedLang === "ro" ? "Abilități active & moveset" : "Active abilities & moveset"}
              </p>
              <p className="mt-1 text-[13px] text-[var(--omni-ink-soft)]">
                {normalizedLang === "ro"
              ...
              {missionCategory ? (
                <p className="mt-1 text-[11px] text-[var(--omni-muted)]">
                  {normalizedLang === "ro"
                    ? `Prioritizăm abilități relevante pentru misiunea ta actuală („${missionCategory}”).`
                    : `We prioritise abilities relevant to your current mission (“${missionCategory}”).`}
                </p>
              ) : null}
            </div>
          </div>
