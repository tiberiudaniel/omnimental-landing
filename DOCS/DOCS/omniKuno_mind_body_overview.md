# Mind–Body Overview – OmniKuno

## Emotional Balance – Body ↔ Mind

# emotional_balance_l3_13_body_to_mind
[SCREEN content]
title: Corpul influențează intensitatea emoțiilor
body: Când glicemia este instabilă sau corpul este flămând ori tensionat, emoțiile devin mai puternice și mai greu de reglat. Nu este „slăbiciune”; este un răspuns fizic.

[SCREEN content]
title: Reacție sau realitate?
body: De multe ori, iritarea, furia sau anxietatea cresc pentru că organismul este în stare de alarmă metabolică.

[SCREEN checkpoint]
title: Observare
steps:
- Gândește-te la o perioadă în care ai reacționat disproporționat.
- Cum era corpul tău atunci? Flămând, obosit, deshidratat?
helper: Uneori emoțiile nu reflectă situația, ci starea corpului.

[SCREEN quiz]
title: Corp și emoții
question: Ce poate amplifica emoțiile?
options:
- Doar gândurile negative.
- Glicemia instabilă, foamea sau oboseala.
correctIndex: 1

[SCREEN reflection]
title: Conștientizare
prompt: „O situație în care corpul mi-a amplificat emoțiile a fost ___.”


# emotional_balance_l3_14_mind_to_body
[SCREEN content]
title: Emoțiile influențează corpul
body: Când ești stresat sau supărat, corpul intră în tensiune: respirația se scurtează, pulsul crește, digestia încetinește.

[SCREEN content]
title: Reglarea emoțiilor prin corp
body: O respirație lentă sau o mică mișcare poate reduce tensiunea și poate calma valul emoțional fără să forțezi nimic mental.

[SCREEN checkpoint]
title: Observă
steps:
- Gândește-te la ultimul moment de furie sau anxietate.
- Unde ai simțit tensiunea în corp?
helper: Emoțiile lăsate nesupravegheate rămân în corp.

[SCREEN quiz]
title: Minte și corp
question: Cum poți calma o emoție ridicată?
options:
- Doar prin analiză mentală.
- Prin reglarea corpului: respirație, relaxare, mișcare blândă.
correctIndex: 1

[SCREEN reflection]
title: Integrare
prompt: „Un gest corporal care îmi calmează emoțiile este ___.”


## Claritate & Focus – Body ↔ Mind

# focus_clarity_l3_13_body_to_mind
[SCREEN content]
title: Corpul influențează focusul
body: Atunci când ești flămând, deshidratat sau ai glicemia oscilantă, creierul prioritizează supraviețuirea, nu atenția.

[SCREEN content]
title: Stabilitate metabolică = claritate
body: Două mese stabile și o hidratare minimă susțin focusul mai mult decât motivația pură.

[SCREEN checkpoint]
title: Observare
steps:
- Gândește-te la ultimul moment în care nu te puteai concentra.
- Cum era corpul tău atunci?
helper: Focusul începe în corp.

[SCREEN quiz]
title: Claritate și corp
question: Ce ajută cel mai mult focusul?
options:
- Să sari peste mese.
- Un corp stabil: hrană, hidratare, respirație.
correctIndex: 1

[SCREEN reflection]
title: Reglați
prompt: „Un gest mic care îmi îmbunătățește focusul este ___.”


# focus_clarity_l3_14_mind_to_body
[SCREEN content]
title: Mintea influențează tensiunea corporală
body: Gândurile grăbite, anxioase sau critique cresc tensiunea musculară și scurtează respirația, reducând claritatea.

[SCREEN content]
title: Atenție și relaxare
body: Când îți domolești ritmul mental, corpul răspunde cu relaxare, iar focusul revine.

[SCREEN checkpoint]
title: Observare
steps:
- Observă cum respiră corpul tău când ești foarte încărcat mental.
helper: Mintea tensionată creează corp tensionat.

[SCREEN quiz]
title: Minte → corp
question: Ce se întâmplă când gândurile se accelerează?
options:
- Corpul se relaxează.
- Corpul intră în tensiune și focusul scade.
correctIndex: 1

[SCREEN reflection]
title: Un gest mental
prompt: „Un gând sau o frază care îmi calmează corpul este ___.”


## Relații & Comunicare – Body ↔ Mind

# relationships_communication_l3_13_body_to_mind
[SCREEN content]
title: Corpul influențează tonul
body: Foamea, oboseala și tensiunea scad empatia și cresc reactivitatea. Nu este „doar personalitate”; este și biochimie.

[SCREEN content]
title: Corp reglat = ton calm
body: O respirație lentă, o gură de apă sau o gustare mică pot reduce impulsivitatea verbală.

[SCREEN checkpoint]
title: Observare
steps:
- Gândește-te la o discuție în care ai reacționat prea tăios.
- Cum era corpul tău în acel moment?
helper: Corpul tău era probabil în stare de alertă.

[SCREEN quiz]
title: Corp & comunicare
question: Ce susține comunicarea calmă?
options:
- Să ignori corpul complet.
- Un corp stabil și regulat metabolic.
correctIndex: 1

[SCREEN reflection]
title: Pregătire
prompt: „Înainte de o discuție importantă, mă ajută dacă ___.”


# relationships_communication_l3_14_mind_to_body
[SCREEN content]
title: Comunicare → tensiune corporală
body: Confruntările și neînțelegerile aprind sistemul nervos: umeri ridicați, stomac încordat, respirație scurtă.

[SCREEN content]
title: Calmul verbal calmează corpul
body: Când alegi un ton mai lent și mai clar, corpul răspunde cu relaxare, chiar dacă subiectul rămâne dificil.

[SCREEN checkpoint]
title: Observare
steps:
- Observă cum reacționează corpul la tonul tău propriu într-o discuție tensionată.
helper: Comunicarea nu e doar mentală, ci și fizică.

[SCREEN quiz]
title: Tonul și corpul
question: Ce liniștește corpul în comunicare?
options:
- Graba și tonul ridicat.
- Ritmul lent, tonul calm și pauzele scurte.
correctIndex: 1

[SCREEN reflection]
title: Micro-reglare
prompt: „Un gest care îmi relaxează corpul în conversații este ___.”


## Energie & Corp – Body ↔ Mind

# energy_body_l3_13_body_to_mind
[SCREEN content]
title: Alimentele influențează energia mentală
body: Zaharurile rapide, mesele foarte grele sau mesele sărite provoacă fluctuații mari de energie și dispoziție.

[SCREEN content]
title: Hrană stabilă, energie stabilă
body: Corpul răspunde bine la obiceiuri regulate: mese relativ stabile, nu haos total.

[SCREEN checkpoint]
title: Observare
steps:
- Când ți-a scăzut energia foarte mult ultima dată?
- Cum arăta ziua ta ca alimentație?
helper: Energia este legată direct de modul în care îți hrănești corpul.

[SCREEN quiz]
title: Energie
question: Ce susține energia stabilă?
options:
- Mese haotice și mult zahăr.
- Ritm mai regulat și alimente echilibrate.
correctIndex: 1

[SCREEN reflection]
title: Mic pas
prompt: „Un gest mic pentru energia mea, legat de hrană, este să ___.”


# energy_body_l3_14_mind_to_body
[SCREEN content]
title: Stresul influențează corpul metabolic
body: Când ești stresat, digestia încetinește, respirația se schimbă, iar corpul intră în modul „alarmă”, chiar dacă stai pe loc.

[SCREEN content]
title: Calmarea minții stabilizează corpul
body: Respirația lentă, o pauză scurtă și un ton mai blând cu tine reduc tensiunea și îmbunătățesc energia pe termen scurt.

[SCREEN checkpoint]
title: Observare
steps:
- Unde simți cel mai des stresul în corp: stomac, piept, gât, umeri?
helper: Stresul prelungit, fără reglare, îți consumă energia.

[SCREEN quiz]
title: Minte și metabolism
question: Ce calmează corpul?
options:
- Ritm mental haotic și auto-critică.
- Gânduri mai liniștite și respirație lentă.
correctIndex: 1

[SCREEN reflection]
title: Reechilibrare
prompt: „Un mod simplu în care îmi pot calma corpul când sunt stresat este să ___.”


## Încredere în Sine – Body ↔ Mind

# self_trust_l3_13_body_to_mind
[SCREEN content]
title: Corpul influențează voința
body: Când ești obosit sau flămând, promisiunile devin greu de respectat. De multe ori nu este lipsă de disciplină, ci lipsă de energie.

[SCREEN content]
title: Stabilitatea corpului crește încrederea
body: Când îți îngrijești corpul, îți este mai ușor să respecți ce ți-ai propus și să simți că poți conta pe tine.

[SCREEN checkpoint]
title: Observare
steps:
- Gândește-te la o promisiune abandonată când erai epuizat(ă).
- Cum arăta starea ta fizică în acele zile?
helper: Voința depinde de rezervele de energie.

[SCREEN quiz]
title: Corp și voință
question: Ce influențează capacitatea de a respecta promisiunile?
options:
- Doar disciplina mentală.
- Și energia și stabilitatea corpului.
correctIndex: 1

[SCREEN reflection]
title: Claritate
prompt: „Îmi pot sprijini promisiunile dacă am grijă să ___.”


# self_trust_l3_14_mind_to_body
[SCREEN content]
title: Dialogul interior influențează corpul
body: Când te critici constant, corpul intră în tensiune; când îți vorbești ca unui aliat, corpul se mai relaxează.

[SCREEN content]
title: Încrederea începe și în corp
body: Câteva cuvinte blânde pot modifica respirația, pulsul și tensiunea musculară.

[SCREEN checkpoint]
title: Observare
steps:
- Observă cum reacționează corpul când îți spui „nu sunt în stare”.
helper: Mintea dură creează corp rigid.

[SCREEN quiz]
title: Voință & corp
question: Ce întărește cel mai mult încrederea în tine?
options:
- Tonul critic constant.
- Tonul aliat și realist, care vede greșeala și propune o ajustare.
correctIndex: 1

[SCREEN reflection]
title: Fraza mea
prompt: „O frază calmă care îmi relaxează corpul și îmi susține încrederea este ___.”


## Discernământ & Decizii – Body ↔ Mind

# decision_discernment_l3_13_body_to_mind
[SCREEN content]
title: Corpul influențează deciziile
body: Când ești flămând, obosit sau foarte tensionat, impulsivitatea crește și capacitatea de analiză calmă scade.

[SCREEN content]
title: Decizii bune cer un corp stabil
body: Câteva înghițituri de apă, o gustare mică sau o scurtă pauză de respirație pot îmbunătăți claritatea deciziei.

[SCREEN checkpoint]
title: Observare
steps:
- Gândește-te la o decizie impulsivă recentă.
- Cum era corpul tău atunci?
helper: Starea corpului poate împinge deciziile spre impuls.

[SCREEN quiz]
title: Decizie & corp
question: Ce susține deciziile calme?
options:
- Tensiunea extremă și foamea.
- Un corp relativ stabil și reglat.
correctIndex: 1

[SCREEN reflection]
title: Condiție prealabilă
prompt: „Pot lua decizii mai bune dacă înainte ___.”


# decision_discernment_l3_14_mind_to_body
[SCREEN content]
title: Mintea influențează digestia și tensiunea
body: Gândurile încărcate despre o decizie grea fac corpul să intre în alertă: stomac strâns, umeri tari, respirație superficială.

[SCREEN content]
title: Claritatea mentală calmează corpul
body: Când îți reduci ritmul gândurilor, clarifici criteriul principal și accepți o parte de incertitudine, corpul se poate relaxa.

[SCREEN checkpoint]
title: Observare
steps:
- Unde simți de obicei deciziile grele în corp?
helper: Corpul reține tensiunea deciziilor neclarificate.

[SCREEN quiz]
title: Minte → corp
question: Ce ajută corpul într-o decizie grea?
options:
- Gânduri rapide, ruminație și presiune mentală.
- Claritate, o pauză scurtă și respirație lentă.
correctIndex: 1

[SCREEN reflection]
title: Integrare
prompt: „Un gest mental care îmi relaxează corpul când am o decizie grea este să ___.”
