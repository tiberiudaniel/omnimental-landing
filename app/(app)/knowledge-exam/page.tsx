...
export default function KnowledgeExamPage() {
  ...
  return (
    <>
      <AppShell
        header={<SiteHeader onAuthRequest={!profile?.id ? goToAuth : undefined} onMenuToggle={() => setMenuOpen(true)} />}
      ...
      >
        ...
        <div className="rounded-2xl border border-[var(--omni-border-soft)] bg-[var(--omni-surface-card)] px-6 py-6 shadow-sm md:px-8 md:py-8">
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[var(--omni-muted)]">{sectionLabel}</p>
          <h1 className="mt-1 text-3xl font-bold text-[var(--omni-ink)]">{String(t("antrenament.oc.quizHeading"))}</h1>
