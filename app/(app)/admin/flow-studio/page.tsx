...
export default function FlowStudioPage() {
  ...
  const stepNodes = useMemo<Node<StepNodeRenderData>[]>(() => {
    ...
    return list;
  }, [expandedStepRenderData, selectedStepNodeId]);
  ...
  }, [edgeIssueMap, edges, nodeRouteById, observedEdgeStatsMap, observedEnabled]);
  const edgesForCanvas = useMemo(() => {
    return [...baseDisplayEdges, ...stepEdgesList];
  }, [baseDisplayEdges, stepEdgesList]);
  ...
  }, [selectedEdgeId, selectedNodeId, setEdges, setNodes, setSelectedEdgeId, setSelectedNodeId]);
  ...
  }, [selectedNodeId, setNodes, setSelectedEdgeId, setSelectedNodeId]);
  ...
  }, [diagnostics.length, edges, flowDoc?.name, flowDoc?.updatedAt, flowDoc?.version, flowNameDraft, flowStats, nodes, selectedFlowId]);
  ...
  const zoomButtonDisabled = flowsTabDisabled || !nodes.length || zoomingToFit;
  const centerButtonDisabled = flowsTabDisabled || !hasSelection || centeringSelection;
      ...
      <div className="min-h-screen bg-[var(--omni-bg-main)] px-4 py-8 text-[var(--omni-ink)] sm:px-6 lg:px-10">
        <div className="flex w-full flex-col gap-6">
          ...
          <div className="flex flex-col gap-5 xl:flex-row">
            <div className={clsx("xl:w-[320px]", routesCollapsed && "xl:w-auto xl:flex-none")}>
              {routesCollapsed ? (
                <CollapsedPanel label="Routes" onExpand={() => setRoutesCollapsed(false)} />
              ) : (
                <RoutesPanel
            ...
            <div className="flex-1 order-first xl:order-none">
              <FlowCanvas
                ...
                onNodeSelect={(node) => {
                  if (node.type === "stepNode") {
                    ...
                    setSelectedNodeId(stepData.parentNodeId);
            ...
            <div className={clsx("xl:w-[360px]", inspectorCollapsed && "xl:w-auto xl:flex-none")}>
              {inspectorCollapsed ? (
                <CollapsedPanel label="Inspector" onExpand={() => setInspectorCollapsed(false)} />
              ) : (
                <InspectorPanel
...
function CollapsedPanel({ label, onExpand }: CollapsedPanelProps) {
  return (
    <div className="flex h-full items-center justify-center rounded-3xl border border-dashed border-[var(--omni-border-soft)] bg-[var(--omni-bg-paper)] p-4 text-center">
      <button
        type="button"
        className="rounded-full border border-[var(--omni-border-soft)] px-4 py-2 text-xs font-semibold"
        onClick={onExpand}
      >
        Arata {label}
      </button>
...
function parseFlowSpecText(text: string): FlowSpecPreview {
  if (!text.trim()) {
...
function normalizeFlowSpecPayload(payload: unknown): FlowSpecPreview {
  ...
  const nodes: FlowSpecNode[] = nodesInput.map((nodeRaw, index) => {
    ...
    const tags = Array.isArray(nodeRaw.tags) ? nodeRaw.tags.filter((tag): tag is string => typeof tag === "string" && Boolean(tag.trim())) : undefined;
    return {
      id,
      routeId,
      routePath,
...
function normalizeSpecTags(tags?: string[] | null, isStart?: boolean): string[] | undefined {
  const normalized = Array.isArray(tags) ? tags.filter((tag): tag is string => typeof tag === "string" && Boolean(tag.trim())).map((tag) => tag.trim()) : [];
...
function normalizeLabelMap(label?: LabelMap | null): LabelMap | undefined {
