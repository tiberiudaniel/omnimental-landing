...
function mapTokensToIndicators(tokens: Record<string, number>, lang: "ro" | "en"): IndicatorCounts {
  ...
  for (const [w, c] of Object.entries(tokens)) {
    ...
    if (lang === "ro") {
      ...
      else if (/greutat|diet|aliment|mancar|slab|nutrit/i.test(w)) add("optimal_weight_management", c);
    } else {
      ...
      else if (/weight|diet|food|nutrition|portion|metabol|crave/i.test(w)) add("optimal_weight_management", c);
    }
  }
  return ic;
}
